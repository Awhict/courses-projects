...
_main:
	...
	mov	DWORD PTR [esp], OFFSET str_hint_input
...
label_test_nxt_char:
	inc	DWORD PTR [esp+ 40]     # 指针向后移动一位，指向下一个要检测的字符
...
label_modify_lower_or_upper:    # 小写字母转大写
	lea	edx, [esp+25]
...
label_modify_upper_to_lower:  # 大写字母转小写 (*** 请完成这段功能代码)
	...
	mov	eax, DWORD PTR [esp+36]
	add	eax, ecx
	mov	BYTE PTR [eax], dl  # 将转换后的小写字母放入新构建的字符串的对应位置
	jmp	label_modify_nxt_char
